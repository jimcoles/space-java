...
import org.jkcsoft.space.intellij.plugin.psi.SpaceElementFactory;
...
class CreatePropertyQuickFix extends BaseIntentionAction {
      ...
      public void run() {
        ...
        if (virtualFiles.size() == 1) {
          ...
          final FileChooserDescriptor descriptor =
              FileChooserDescriptorFactory.createSingleFileDescriptor(SpaceFileType.INSTANCE);
  ...
  private void createProperty(final Project project, final VirtualFile file) {
    new WriteCommandAction.Simple(project) {
      @Override
      public void run() {
        SpaceFile spaceFile = (SpaceFile) PsiManager.getInstance(project).findFile(file);
        ASTNode lastChildNode = spaceFile.getNode().getLastChildNode();
        // TODO: Add another check for CRLF
        if (lastChildNode != null/* && !lastChildNode.getElementType().equals(SimpleTypes.CRLF)*/) {
          spaceFile.getNode().addChild(SpaceElementFactory.createCRLF(project).getNode());
        }
        // IMPORTANT: change spaces to escaped spaces or the new node will only have the first word for the key
        SimpleProperty property = SpaceElementFactory.createProperty(project, key.replaceAll(" ", "\\\\ "), "");
        spaceFile.getNode().addChild(property.getNode());
